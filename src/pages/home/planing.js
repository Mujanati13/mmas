...
export default function Planing({navigation}) {
  const [isScreen1Active, setIsScreen1Active] = useState(false);
  ...
  const [date, setDate] = useState(new Date()); // Définir la valeur de date initiale
  const [showDatePicker, setShowDatePicker] = useState(false); // Contrôler la visibilité du sélecteur de date
  ...
  const toggleScreen = () => {
    setIsScreen1Active(!isScreen1Active);
  };
  ...
  const handleDateChange = (event, selectedDate) => {
    ...
    const selectedDayOfWeek = currentDate.getDay(); // Get the day of the week (0-6, where 0 is Sunday)
    console.log(selectedDayOfWeek); // Log the selected day of the week
    setDate(currentDate);
    setShowDatePicker(false); // Fermer le sélecteur de date après la sélection de la date
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // const response = await fetchSessionsByStudentAndDay(currentis, date);
  ...
  return (
    <SafeAreaView
      ...
      <TouchableWithoutFeedback onPress={toggleleMenuClose}>
        <View style={{padding: 20}}>
          <View
            ...
            <Text style={{fontSize: 20, color: 'black'}}>Planning</Text>
          ...
          {/* <View
            ...
            <Text
              style={{
                fontSize: 20,
                color: !isScreen1Active ? 'black' : '#808080',
                fontWeight: 500,
              }}>
              Cours
            </Text>
            <Switch value={isScreen1Active} onValueChange={toggleScreen} />
            <Text
              style={{
                fontSize: 20,
                color: !isScreen1Active ? '#808080' : 'black',
                fontWeight: isScreen1Active ? 600 : 600,
              }}>
              Présence
            </Text>
          </View> */}
      ...
      {/* <Text style={{fontSize:20 , padding:20}}>Planing</Text> */}
      <MyAgenda id={currentis} />
      <Footer activeScreen={'Planing'} />
    </SafeAreaView>
  );
}
